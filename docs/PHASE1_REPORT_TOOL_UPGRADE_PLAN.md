# 第一阶段：报告工具增强计划（功能清单与界面设计 v2）

日期：2026-09-12 ｜ 版本：v2（按「落实到具体新功能 + 前端界面」重写，替代 v1 架构稿）
阶段定位：**先做**。在现有「写报告」主线上做厚——素材采集、持续追踪、告警、地域地图、数据表格。
后续阶段见 `docs/PHASE2_OPINION_WORKBENCH_PLAN.md`（舆情工作台，主动型产品形态）。
...
> ⚠️ **2026-09-20 部分作废**：以上「不做独立大屏 / 实时数据流 dashboard」两条已被
> `docs/PHASE2_OPINION_WORKBENCH_PLAN.md`（第二阶段：舆情工作台升级计划书 v1）推翻 ——
> 产品方向改为主动型舆情工作台，新增今日热点 / 舆情监测 / 监测分析 / 舆情地图 / 监测大屏。
> 本文件其余功能（F1–F15）仍然有效，其继承与覆盖关系见该计划书第零章。
